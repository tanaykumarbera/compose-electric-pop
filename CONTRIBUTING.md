...
### KDoc screenshot tables

Every component's KDoc carries a delimited screenshot table that mirrors the goldens on disk. The block looks like:

```
<!-- screenshots:start (auto-generated, do not edit) -->
| Scenario | Light | Dark |
| --- | --- | --- |
| <scenario> | ![](.../snapshots/Pop<X>_<scenario>_light.png) | ![](.../snapshots/Pop<X>_<scenario>_dark.png) |
<!-- screenshots:end -->
```

The block is regenerated by `./gradlew :library:syncScreenshotKdoc` from the snapshots in `library/src/desktopTest/snapshots/`. Do not edit it by hand — your changes will be overwritten. Add or rename a scenario via the screenshot test, then re-run the task.

Install the pre-commit hook once per clone so commits with a stale block fail locally instead of in CI:

```bash
./scripts/install-hooks.sh
```

CI runs the same check on every PR (`syncScreenshotKdoc` + `git diff --exit-code library/src/commonMain`).
